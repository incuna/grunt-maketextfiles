exports.init = function(grunt, options) {
    ...
    settings.watchFiles = _.pluck(settings.filePatterns, 'filePattern');
    ...
    function getTextFilePatterns(modulePaths, fileTypes) {
        ...
        function getRealModulePath(modulePath) {
            ...
            if (config && config.packages) {
                ...
                matchingPackage = _.find(config.packages, function(packageItem) {
                    return (modulePath.indexOf(packageItem.name + '/') === 0);
                });
                if (matchingPackage) {
                    //modulePath starts with this package name, so we need to replace it with real path
                    realModulePath = modulePath.replace(matchingPackage.name, matchingPackage.location);
                }
            }
